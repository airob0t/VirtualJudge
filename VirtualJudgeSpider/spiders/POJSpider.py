...
class POJ(Base):
    ...
    def login_webside(self, *args, **kwargs):
        ...
        post_data = {'user_id1': kwargs['account'].get_username(),
                     ...
                     'url': '/'}
        try:
            res1 = self.request.get(url=login_page_url)
            self.cookies = res1.cookies
            res2 = self.request.post(url=login_link_url, data=post_data)
    ...
    def submit_code(self, *args, **kwargs):
        ...
        try:
            ...
            post_data = {'problem_id': pid,
                         ...
                         'submit': 'Submit',
                         'encoded': '1'}
            res = self.request.post(url=url, data=post_data)
    ...
    def get_next_problem_id(self):
        ...
        tags_a = center.find_all('a')
        pages = [tag_a.find('font').string for tag_a in tags_a]
        try:
            for page in pages:
                listpage_url = url+'?volume='+str(page)
                listpage = self.request.get(listpage_url)
                soup = BeautifulSoup(listpage.text, 'lxml')
                table = soup.find('table', attrs={'class': 'a'})
                trs = table.find_all('tr', attrs={'align': 'center'})
                problems = [tr.find('td').string for tr in trs]
